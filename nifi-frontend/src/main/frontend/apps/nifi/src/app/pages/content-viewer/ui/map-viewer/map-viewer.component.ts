...
export class MapViewer implements AfterViewInit, OnDestroy {
    ...
    activeGeoType: 'Features' | 'Tiles' | null = null;
    ...
    private initializeMap(): void {
        ...
        this.map.on('load', () => {
            if (!this.map) return;
            this.map.resize();
            this.addDebugGridSource(); // Restored async logic
            this.registerSelectionLogic();
            if (this.ref) {
                this.loadDataByMetadata(this.ref);
            }
            setTimeout(() => this.map?.resize(), 100);
        });
    }

    private loadDataByMetadata(ref: string): void {
        const metadataUrl = `/${this.contextPath}/api/geometry/metadata?ref=${encodeURIComponent(ref)}`;

        this.http.get<MapMetadata>(metadataUrl).subscribe({
            next: (meta) => {
                this.activeGeoType = meta.geoType;
                this.clearNifiLayers();
    ...
    private clearNifiLayers(): void {
        ...
        style.layers?.forEach((l) => {
            if (l.id.startsWith('local-') || l.id.startsWith('h-') || l.id === 'nifi-raster-layer') {
                this.map?.removeLayer(l.id);
            }
        });

        if (this.map.getSource('nifi-source')) this.map.removeSource('nifi-source');
        if (this.map.getSource('nifi-raster-source')) this.map.removeSource('nifi-raster-source');
    ...
    private addNifiRasterSource(ref: string): void {
        if (!this.map) return;
        const url = `/${this.contextPath}/api/geometry/tiles_img/{z}/{x}/{y}?ref=${encodeURIComponent(ref)}`;

        this.map.addSource('nifi-raster-source', {
            type: 'raster',
            tiles: [url],
            tileSize: 256
        });
        ...
        this.map.addLayer({
            id: 'nifi-raster-layer',
            type: 'raster',
            source: 'nifi-raster-source',
    ...
    private registerSelectionLogic(): void {
        if (!this.map) return;
        this.map.on('click', (e) => {
            if (this.activeGeoType !== 'Features') return;
    ...
    // RESTORED: Your working GridCell logic
    ...
    toggleNiFiLayer(): void {
        ...
        const v = this.layerVisibility.local ? 'visible' : 'none';
        if (this.activeGeoType === 'Features') {
            ['local-polygons', 'local-lines', 'local-points'].forEach((id) => {
                if (this.map?.getLayer(id)) this.map.setLayoutProperty(id, 'visibility', v);
            });
        } else if (this.map?.getLayer('nifi-raster-layer')) {
            this.map.setLayoutProperty('nifi-raster-layer', 'visibility', v);
        }
    }
    ...
    resetHighlights(): void {
        ...
        ['h-poly', 'h-line', 'h-point'].forEach((id) => {
            if (this.map?.getLayer(id)) this.map.setFilter(id, ['==', ['get', 'feature_id'], -1]);
        });
        this.selectedFeatureProperties = null;
        this.cdr.detectChanges();
    }
